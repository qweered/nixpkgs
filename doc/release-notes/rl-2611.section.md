...
- `rustPlatform.importCargoLock` now requires `extraRegistries` keys to match
  the source identifiers in {file}`Cargo.lock`. Prefix standard registry index
  URLs with `registry+`; sparse registry keys already include their `sparse+`
  prefix.
...
- Module functions must now be native Nix functions. Callable attribute sets
  using `__functor` are treated as attribute-set modules and are no longer
  invoked by the module system.
