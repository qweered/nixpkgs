...
- Four hardening flags are now enabled by default for all packages built with the
  C/C++ compiler and linker wrappers, following the
  ...
  [`securitywarnings`](#securitywarnings) enables the diagnostics recommended by
  the [OpenSSF Compiler Options Hardening Guide for C and C++](https://best.openssf.org/Compiler-Hardening-Guides/Compiler-Options-Hardening-Guide-for-C-and-C++.html),
  namely `-Wall -Wextra -Wimplicit-fallthrough` (plus `-Wtrampolines
  -Wbidi-chars=any` on GCC), and makes the C constraint violations
  `-Werror=implicit`, `-Werror=int-conversion` and
  `-Werror=incompatible-pointer-types` fatal. The latter three are already
  errors by default in C23, GCC 14 and Clang 16.

  [`nowerror`](#nowerror) stops a package's build system from deciding which
  diagnostics are fatal, so that upstream projects developing with `-Werror` no
  longer fail to build on any newly introduced compiler diagnostic. It appends
  `-Wno-error` to cancel a blanket `-Werror`, and additionally removes
  `-Werror`, `-Werror=<warning>`, `-Werror-<warning>` and `-w` from the
  package's own command line — appending a flag cannot cancel the specific form,
  since GCC and Clang resolve conflicting `-W` options by specificity rather
  than by position, and `-w` stops warnings being emitted at all so nothing
  remains for a `-Werror=` to escalate. `-Wno-error=<warning>` is left in place.

  Packages that relied on `-w` will produce noticeably more build log output,
  and will now fail if they contain a format-string defect. Such packages should
  request `-w` through `env.NIX_CFLAGS_COMPILE`, which is not filtered.

  Packages that suppressed one of the newly fatal diagnostics by passing
  `-Wno-error=<warning>` through `CFLAGS`, `makeFlags`, `configureFlags` or
  `cmakeFlags` must move it to `env.NIX_CFLAGS_COMPILE`, which is the only
  channel appended after the hardening flags.

  [`noexecstack`](#noexecstack) passes `-z noexecstack` to the linker, marking
  the stack of the produced binary non-executable. Note that a program which
  genuinely requires an executable stack — in practice, one using GCC's nested
  function trampolines — will build fine and fail at runtime; `-Wtrampolines`
  from `securitywarnings` reports that construct at compile time.

  [`nodeletenullpointerchecks`](#nodeletenullpointerchecks) passes
  `-fno-delete-null-pointer-checks`, which stops the optimiser from removing a
  null check on the grounds that an earlier dereference already made the pointer
  non-null. It only preserves code that would otherwise be discarded, so it
  cannot change the behaviour of a correct program.

  Any of these can be turned off per package with `hardeningDisable`, and
  individual diagnostics can be re-permitted with `env.NIX_CFLAGS_COMPILE =
  "-Wno-error=<warning>"`.
