...
- The kernel-related `boot.*` options have been moved under `boot.kernel.*` for better discoverability and consistency with the existing `boot.kernel.{enable,sysctl,features}` namespace:
  - `boot.kernelPackages` → `boot.kernel.packages`
  - `boot.kernelPatches` → `boot.kernel.patches`
  - `boot.kernelParams` → `boot.kernel.params`
  - `boot.kernelModules` → `boot.kernel.modules`
  - `boot.blacklistedKernelModules` → `boot.kernel.blacklistedModules`
  - `boot.extraModulePackages` → `boot.kernel.extraModulePackages`
  - `boot.consoleLogLevel` → `boot.kernel.consoleLogLevel`

  The old names continue to work via aliases but emit a deprecation warning; please migrate your configuration. Note that `boot.initrd.kernelModules` and `boot.initrd.availableKernelModules` are unaffected.

- `boot.extraModprobeConfig` has been moved to `boot.modprobeConfig.extra` to live alongside the other `boot.modprobeConfig.*` options. The old name keeps working through an alias but emits a deprecation warning.
