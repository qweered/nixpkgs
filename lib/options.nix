...
rec {
  ...
  */
  mergeEqualOption =
    loc: defs:
    # Return early if we only have one element. This also makes it work for
    # functions, because the foldl' below would try to compare the first element
    # with itself, which is false for functions.
    if length defs == 1 then
      (head defs).value
    else if defs == [ ] then
